...
export default function CampaignDetail() {
  ...
  async function verifyAddresses(targetHomeIds = null) {
    setVerifyResults(null);
    setMailMsg('');
    setErr('');
    const all = data?.homes?.filter(isPreviewMailable) || [];
    const rendered = Array.isArray(targetHomeIds) && targetHomeIds.length
      ? all.filter((h) => targetHomeIds.includes(h.id))
      : all;
    if (!rendered.length) {
    ...
    setVerifyBusy(true);
    try {
      const res = await api.verifyCampaignAddresses(id, {
        unmailedOnly: true,
  ...
  async function sendMail(live = false, targetHomeIds = null) {
    setMailMsg('');
    setMailPreviewLinks([]);
    setErr('');
    const all = data?.homes?.filter(live ? isLiveMailable : isPreviewMailable) || [];
    const rendered = Array.isArray(targetHomeIds) && targetHomeIds.length
      ? all.filter((h) => targetHomeIds.includes(h.id))
      : all;
    if (!rendered.length) {
  ...
  const selectedMailable = Boolean(selectedHome && isPreviewMailable(selectedHome));
  const selectedLiveMailable = Boolean(selectedHome && isLiveMailable(selectedHome));
  const selectedStreet = selectedHome ? streetLabel(selectedHome.address) : '';
  const mailTargetIds = selectedMailable ? [selectedHome.id] : null;
  const canMailPanel = selectedHome ? selectedMailable : mailableCount > 0;
  const canLivePanel = selectedHome ? selectedLiveMailable : liveMailableCount > 0;
        ...
        <aside className="or-side">
          ...
          <div className="mail-panel">
            ...
            <p className="mail-cost">
              {selectedMailable
                ? `Mailed design quote to ${selectedStreet} · ~$1.00–$1.50 (print + postage)`
                : mailableCount > 0
                  ? `Mail ${mailableCount} rendered home(s) · ~$${(mailableCount * 1).toFixed(2)}–$${(mailableCount * 1.5).toFixed(2)} (print + postage). Click a home below to mail that address.`
                  : 'Make quotes first — then send a mailed design quote to any address.'}
            </p>
            {selectedHome && (
              <p className="or-status" style={{ marginTop: 0, marginBottom: 8, fontSize: 12 }}>
                Selected: {selectedHome.address}
                {!selectedMailable && ' — make a quote first to mail this address'}
                {' · '}
                <button
                  type="button"
                  className="or-link-btn"
            ...
            <button
              type="button"
              className="btn ghost block sm"
              disabled={verifyBusy || mailBusy || !canMailPanel}
              onClick={() => verifyAddresses(mailTargetIds)}
            >
              {verifyBusy
                ? 'Checking…'
                : selectedMailable
                  ? `Check ${selectedStreet}`
                  : 'Check addresses (USPS)'}
            </button>
            <button
              type="button"
              className="btn ghost block"
              disabled={mailBusy || verifyBusy || !canMailPanel}
              onClick={() => sendMail(false, mailTargetIds)}
              style={{ marginTop: 8 }}
            >
              {mailBusy
                ? 'Sending…'
                : selectedMailable
                  ? `Preview postcard — ${selectedStreet}`
                  : `Preview ${mailableCount || ''} PDFs`.trim()}
            </button>
            {selectedMailable && mailableCount > 1 && (
              <button
                type="button"
                className="btn ghost block sm"
                style={{ marginTop: 8 }}
              ...
              >
                Preview all {mailableCount} PDFs
              </button>
            )}
            {mailedCount > 0 && (
              <button
            ...
            {mailStatus?.readyForLive && (
              <button
                ...
                style={{ marginTop: 8 }}
                disabled={mailBusy || verifyBusy || !canLivePanel}
                onClick={() => sendMail(true, selectedLiveMailable ? [selectedHome.id] : null)}
              >
                {selectedLiveMailable
                  ? `Send postcard to ${selectedStreet}`
                  : 'Send live via Lob'}
              </button>
            )}
            {mailStatus && !mailStatus.readyForLive && mailStatus.hints?.length > 0 && (
              <p className="muted" style={{ fontSize: 11, marginTop: 10, lineHeight: 1.45 }}>
                Physical mail: {mailStatus.hints.join(' · ')}
              </p>
            )}
          ...
          {homes.length > 0 && (
            <div className="card card-table">
              ...
              <table className="table">
                <thead>
                  <tr><th>Address</th><th>Owner</th><th>Est. quote</th><th>Status</th><th>Postcard</th></tr>
                </thead>
                <tbody>
                  {homes.slice(0, 50).map((h) => {
                    const previewOk = isPreviewMailable(h);
                    const liveOk = isLiveMailable(h);
                    return (
                    <tr
                      key={h.id}
                      className={selectedHomeId === h.id ? 'is-selected' : ''}
                      style={{ cursor: 'pointer' }}
                      onClick={() => setSelectedHomeId((cur) => (cur === h.id ? null : h.id))}
                    >
                      <td>{h.address}</td>
                        ...
                        {h.mail_status === 'sent' ? (
                          ...
                          <div className="or-mail-actions">
                            <button
                              ...
                              disabled={mailBusy || verifyBusy || !mailTemplateId}
                              onClick={() => {
                            ...
                            {mailStatus?.readyForLive && liveOk && (
                              <button
                                type="button"
                                className="btn sm"
                                disabled={mailBusy || verifyBusy || !mailTemplateId}
                            ...
                            {h.mail_status === 'mail_demo' && (
                              <span className="muted" style={{ fontSize: 11 }}>PDF ready</span>
                            )}
                          </div>
                        ) : (
                          <span className="muted" style={{ fontSize: 12 }}>
                            {h.mail_status || (h.render_id ? '—' : 'Make quote first')}
                          </span>
                        )}
                      </td>
                    </tr>
